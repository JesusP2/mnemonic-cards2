import type { Env } from '@/types.js';
import { DrizzleSQLiteAdapter } from '@lucia-auth/adapter-drizzle';
import { Lucia } from 'lucia';
import { ulidFactory } from 'ulid-workers';
import { buildTursoClient } from './db/pool.js';
import { sessionTable, userTable } from './db/schema/index.js';

export function createLuciaInstance(envs: Env['Variables']) {
  const db = buildTursoClient(envs);
  const adapter = new DrizzleSQLiteAdapter(db, sessionTable, userTable);
  const lucia = new Lucia(adapter, {
    sessionCookie: {
      attributes: {
        secure: envs.ENVIRONMENT === 'PRODUCTION', // set `Secure` flag in HTTPS
      },
    },
    getUserAttributes: (attributes) => {
      return {
        username: attributes.username,
        email: attributes.email,
      };
    },
  });
  return lucia;
}

export async function hashPassword(password: string) {
  const textEncoder = new TextEncoder();
  const digest = await crypto.subtle.digest(
    {
      name: 'SHA-256',
    },
    textEncoder.encode(password),
  );
  return btoa(String.fromCharCode(...new Uint8Array(digest)));
}

export async function comparePasswords(password1: string, password2: string) {
  const hashedPassword = await hashPassword(password2);
  return hashedPassword === password1;
}

export function createUlid() {
  const ulid = ulidFactory();
  return ulid();
}

declare module 'lucia' {
  interface Register {
    Lucia: ReturnType<typeof createLuciaInstance>;
    DatabaseUserAttributes: {
      username: string;
      email?: string | null;
    };
  }
}
